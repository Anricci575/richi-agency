...
export const PricingTicketHorizontal = () => {
  ...
  return (
    <div className="mt-12 max-w-5xl mx-auto flex h-auto min-h-[300px] group transition-transform duration-500 hover:-translate-y-2 px-4">
      
      {/* LEFT STUB (Tear-off part) */}
      <div className={`w-24 md:w-48 bg-[#0a0a0a] rounded-l-3xl border border-white/10 border-r-0 relative p-4 md:p-6 flex flex-col justify-between overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.5)] transition-all duration-[1000ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] origin-right z-20 ${isTorn ? '-translate-y-32 -translate-x-4 rotate-[-12deg] opacity-0 pointer-events-none' : ''}`}>
        ...
        <div className="w-full flex justify-between items-start opacity-70">
           <div className="flex gap-[2px] h-20 md:h-32">
             {[...Array(12)].map((_,i) => <div key={i} className="bg-white" style={{ width: Math.random()*3+1 + 'px' }}></div>)}
           </div>
           <div className="writing-vertical text-[10px] font-mono text-gray-500 tracking-widest uppercase" style={{ writingMode: 'vertical-rl', textOrientation: 'mixed', transform: 'rotate(180deg)' }}>TKT-AUTO-99</div>
        </div>
      ...
      {/* VERTICAL SEPARATOR */}
      <div 
        className={`w-8 md:w-10 bg-[#0a0a0a] border-y border-white/10 flex flex-col items-center justify-center relative z-10 transition-opacity duration-300 ${isTorn ? 'opacity-0' : 'opacity-100'}`}
        style={{
          maskImage: 'radial-gradient(circle at 50% 0%, transparent 12px, black 12.5px), radial-gradient(circle at 50% 100%, transparent 12px, black 12.5px)',
          maskSize: '100% 51%',
          maskRepeat: 'no-repeat',
          maskPosition: 'top, bottom',
          WebkitMaskImage: 'radial-gradient(circle at 50% 0%, transparent 12px, black 12.5px), radial-gradient(circle at 50% 100%, transparent 12px, black 12.5px)',
          WebkitMaskSize: '100% 51%',
          WebkitMaskRepeat: 'no-repeat',
          WebkitMaskPosition: 'top, bottom'
        }}
      >
        <div className="h-full w-[2px] border-r-[2px] border-dashed border-white/20 my-6"></div>
      </div>

      {/* RIGHT MAIN BODY */}
      <div className={`flex-1 bg-gradient-to-r from-[#0a0a0a] to-[#140800] rounded-r-3xl border border-white/10 border-l-0 p-6 md:p-12 relative overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.5)] transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-10 ${isTorn ? 'translate-x-4' : ''}`}>
        ...
        <div className="relative z-10 flex flex-col lg:flex-row justify-between gap-8 h-full">
           <div className="flex-1">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-orange-500/40 bg-orange-500/10 text-orange-500 font-pixel text-[9px] uppercase mb-4 tracking-widest shadow-[0_0_15px_rgba(249,115,22,0.1)]">
                <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span>
                Servicio Independiente
              </div>
              <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tight mb-4 text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">
                Automatización & Bots
              </h3>
              <p className="text-gray-400 text-xs md:text-sm font-light max-w-xl mb-8 leading-relaxed">
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-gray-300 font-mono">
                <li className="flex items-center gap-3"><span className="text-orange-500 text-lg leading-none">❖</span> Bots a medida (Telegram/WA)</li>
                <li className="flex items-center gap-3"><span className="text-orange-500 text-lg leading-none">❖</span> Automatización de Procesos</li>
                <li className="flex items-center gap-3"><span className="text-orange-500 text-lg leading-none">❖</span> IA (Cerebros Virtuales)</li>
                <li className="flex items-center gap-3"><span className="text-orange-500 text-lg leading-none">❖</span> Bases de Datos y CRM</li>
              </ul>
           </div>
           
           <div className="flex flex-col justify-end items-start lg:items-end min-w-[200px] border-t lg:border-t-0 lg:border-l border-white/10 pt-6 lg:pt-0 lg:pl-8">
              <div className="text-left lg:text-right w-full mb-6">
                <div className="text-[10px] font-mono text-gray-500 tracking-widest uppercase mb-1">Inversión Única</div>
                <div className="flex items-baseline justify-start lg:justify-end gap-1">
                  ...
                  <span className="text-sm font-mono text-orange-500">USD</span>
                </div>
                <div className="text-[9px] text-orange-500/80 font-pixel mt-2 tracking-widest">&gt; ENTREGA: 4 DÍAS HÁBILES</div>
              </div>
              
              <button onClick={handleTear} className="w-full py-4 bg-orange-500/10 hover:bg-orange-500 border border-orange-500 text-orange-500 hover:text-black text-[10px] font-pixel uppercase tracking-widest rounded-xl text-center transition-all shadow-[0_0_20px_rgba(249,115,22,0.15)] hover:shadow-[0_0_30px_rgba(249,115,22,0.4)]">
