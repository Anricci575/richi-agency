...
function PixelSwap({
  ...
  useEffect(() => {
    ...
    if (!to || !source || !frozenGrid.pixels.length || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      finish();
      return;
    }
  ...
  return (
    ...
    >
      ...
      {children && (
        <div className="pixel-swap__children relative z-10 w-full h-full pointer-events-none">
          {children}
        </div>
      )}
    </div>
